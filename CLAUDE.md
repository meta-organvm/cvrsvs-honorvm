...
**Org:** `meta-organvm` | **Repo:** `cvrsvs-honorvm`

### Edges
- **Produces** → `ALL`: governance-library
- **Produces** → `ALL`: seed-specification
- **Produces** → `ALL`: schema-package
- **Produces** → `ALL`: interop-adapters
- **Consumes** ← `META-ORGANVM`: governance-source
- **Consumes** ← `META-ORGANVM`: schema

### Siblings in Meta
`.github`, `organvm-corpvs-testamentvm`, `alchemia-ingestvm`, `schema-definitions`, `organvm-engine`, `system-dashboard`, `organvm-mcp-server`, `praxis-perpetua`, `stakeholder-portal`, `materia-collider`, `organvm-ontologia`, `vigiles-aeternae--agon-cosmogonicum`
